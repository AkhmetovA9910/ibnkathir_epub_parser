...
`parse_epub.py` читает EPUB, проходит по файлам сур (`content/s2.xhtml` … `content/s115.xhtml`), извлекает из каждого блока `<div class="verse">` номер аята и параграфы тафсира, и сохраняет каждую суру в отдельный файл `output/NNN.json`.
